...
def build_gompertz_model(strain_data, strain_names, pesticide_indices,
    ...
    with pm.Model() as model:
        ...
        # Design history: an attempt to narrow per-strain sigma to
        # max(floor, rel_err * refined) when identifiable (Step 3 of
        # next_steps_plan.md) produced 600 NUTS divergences and CIs 3-5x
        # WIDER than the pre-refactor baseline. Root cause: the refined-μ
        # point estimate comes from truncation-MLE on one window, but the
        # Bayesian likelihood uses the full observation vector, so the data
        # pulls away from the overly-tight prior → sampler stalls at
        # max_treedepth. Reverted to the wide population prior; only the
        # refined-mean shift below is retained.
        ...
        # Gate each parameter on its own identifiability flag; shift the prior
        # MEAN toward the refined value. Sigma narrowing was tried and
        # reverted (see note above the sigma_*_strain declarations).
