...
**Informative priors from per-parameter refinement.** When a strain's refined per-parameter estimate (Section 2.5.1) was classified as identifiable, its prior mean was shifted toward the refined value and the per-strain spread was tightened to $\sigma_{\theta,\text{strain}}^{(i)} = \max(\text{floor}_\theta,\; \text{rel\_err}_\theta^{(i)} \cdot \theta_{\text{refined}}^{(i)})$, with floors $\text{floor}_A = 0.05$, $\text{floor}_{\mu_m} = 0.01$, $\text{floor}_\lambda = 0.2$ to prevent under-coverage. Strains with weakly identifiable or unidentifiable parameters retained the population-level prior for that parameter. This brings the truncation-informed accuracy gains of Section 2.5.1 into the hierarchical inference.
